...
export const getAppUserProfile = async (
    ...
    try {
        ...
        if (error && error.code === "PGRST116") { // PGRST116: Row not found
            ...
            const isZohoOrOrg = email.endsWith("@zoho.com") ||
                email.endsWith("@zoho.in") ||
                email.endsWith("@southwallsecurity.com");

            const newUserProfile = {
                id: supabaseUser.id,
                name: supabaseUser.user_metadata.name || "New User",
                email: email,
                role_id: isZohoOrOrg ? "employee" : "unverified",
            };
            ...
            if (insertError) {
                console.error(
                    "Error creating user profile on-the-fly:",
                    insertError.message,
                    insertError,
                );
...
const signUpWithPassword = async (
    ...
    const { email, password, options } = credentials as {
        email: string;
        password?: string;
        options?: any;
    };
...
const approveUser = async (userId: string, newRole: string) => {
    return await supabase.from("users").update({ role_id: newRole }).eq(
        "id",
        userId,
    );
};
