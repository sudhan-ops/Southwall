        ...
        <div className={`relative notification-bell ${className}`}>
            ...
            {isOpen && createPortal(
                <>
                    ...
                    <div
                        ref={panelRef}
                        className={`fixed inset-x-4 top-20 bottom-20 z-[100] flex flex-col bg-white ${isBlue ? 'dark:!bg-[#0f172a]' : 'dark:!bg-[#0d1f12]'} rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 md:absolute md:inset-auto md:top-16 md:right-4 md:mt-2 md:w-[400px] md:h-auto md:max-h-[600px] md:bottom-auto animate-in fade-in zoom-in-95 duration-200 overflow-hidden`}
                        style={{ position: window.innerWidth >= 768 ? 'absolute' : 'fixed' }}
                    >
                        {/* Header */}
                        <div className={`flex items-center justify-between p-5 border-b border-gray-200 dark:border-white/10 bg-gray-50 ${isBlue ? 'dark:!bg-[#0f172a]' : 'dark:!bg-[#0d1f12]'}`}>
                            <div>
                            ...
                            <div className="flex items-center gap-2">
                                {unreadCount > 0 && (
                                    <span
                                        ...
                                        className={`relative isolate overflow-hidden before:!content-none after:!content-none text-xs h-8 px-4 flex items-center justify-center rounded-3xl ${isBlue ? '!bg-blue-600 hover:!bg-blue-700 !text-white' : '!bg-[#32CD32] hover:!bg-[#28a428] !text-[#0D1A0D]'} !font-bold cursor-pointer transition-colors`}
                        ...
                        {/* Content */}
                        <div className={`flex-1 overflow-y-auto custom-scrollbar bg-white ${isBlue ? 'dark:!bg-[#0f172a]' : 'dark:!bg-[#0d1f12]'} dark:[color-scheme:dark]`}>
                                    ...
                                    {notifications.slice(0, 10).map((notif) => (
                                        <div
                                            ...
                                            className={`group flex items-start gap-4 p-5 cursor-pointer transition-all duration-200 ${!notif.isRead
                                                ? (isBlue ? 'bg-blue-50 dark:!bg-[#0f172a] hover:bg-blue-100 dark:hover:!bg-[#1e293b]' : 'bg-emerald-50 dark:!bg-[#0d1f12] hover:bg-emerald-100 dark:hover:!bg-[#152b1b]')
                                                : (isBlue ? 'bg-white dark:!bg-[#0f172a] hover:bg-gray-50 dark:hover:!bg-[#1e293b]' : 'bg-white dark:!bg-[#0d1f12] hover:bg-gray-50 dark:hover:!bg-[#152b1b]')
                                                }`}
                                        >
                                            <div className={`flex-shrink-0 p-2.5 rounded-full ${!notif.isRead ? (isBlue ? 'bg-white dark:bg-[#1e293b] shadow-sm' : 'bg-white dark:bg-[#1a2e1a] shadow-sm') : (isBlue ? 'bg-gray-100 dark:bg-[#1e293b]' : 'bg-gray-100 dark:bg-[#1a2e1a]')
                                                }`}>
